...
import { initializeModelList } from './modelList.js';
...
map.on('style.load', () => {
  ...
  const modelOrigin = modelState.position;
  const modelAltitude = 0;
  const modelRotate = modelState.rotation;
  ...
  const customLayer = {
    ...
    onAdd: function (map, gl) {
      ...
      loader.load(
        ...
        gltf => {
          ...
          setModelRef(gltf.scene);
          window.modelRef = gltf.scene; // Make it accessible globally

          // Initialize the model list after the model is loaded
          initializeModelList();
    ...
    render: function (gl, matrix) {
      ...
      this.camera.matrixWorld.copy(this.camera.projectionMatrix);
  ...
  setCustomLayerRef(customLayer); // Store reference to the custom layer
...
// Initialize AR without requesting permissions
function initAR() {
...
// Handle initial touch start and request permissions if needed
async function handleTouchStart(event) {
  // Check if we need to request device orientation permission
  if (
    ...
    // No permission needed, start drawing directly
    startDrawing(event);
  }
}
